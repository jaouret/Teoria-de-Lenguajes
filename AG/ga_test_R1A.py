...
# La ecuación tiene 3 entradas (x1 a x3) y 3 constantes (a,b,c) como se muestra.
# Los valores de las entradas son (x1, x2, x3) = (1,2,3).
# Estamos buscando encontrar las constantes a b c que resuelvan dicha ecuación.

import numpy
import ga

# Al principio, creemos una lista de las 3 entradas y variables para contener el número de constantes
# de la siguiente manera:

equation_inputs = [1,2,3]
# cantidad constantes a buscar
num_weights = 3

# El siguiente paso es definir la población inicial.
# Según el número de constantes, cada cromosoma (solución o individuo) en la población definitivamente tendrá 6 genes,
# un gen para cada constante.
# Pero la pregunta es ¿cuántas soluciones por población?
...
# otra para contener el tamaño de la población y, finalmente, una variable que contiene la población
# inicial real:
...
# La población tendrá un cromosoma sol_per_pop donde cada cromosoma tiene genes con constante num_weights.
...
# De acuerdo con los parámetros seleccionados, tendrá forma (10,3).
# Eso es 8 cromosomas y cada uno tiene 3 genes, uno para cada contante.
...
for generation in range(num_generations):
    ...
    fitness = ga.cal_pop_fitness2(equation_inputs, new_population)
    ...
    fitness_tmp = numpy.sum(new_population*equation_inputs, axis=1) - 10
    fitness_tmp_idx = (numpy.abs(fitness_tmp - 0)).argmin()
    best_fitness = fitness_tmp[fitness_tmp_idx]
    print("MEJOR RESULTADO PARA ESTA GENERACION : ", best_fitness)

fitness = ga.cal_pop_fitness2(equation_inputs, new_population)
fitness_idx = (numpy.abs(fitness - 0)).argmin()
best_fitness = fitness[fitness_idx]
print("MEJOR SOLUCION: ", new_population[fitness_idx, :])
print("MEJOR APTITUD : ", best_fitness+10)
