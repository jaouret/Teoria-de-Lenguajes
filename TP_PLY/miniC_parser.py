...
from miniC_lexer_R1 import tokens
import miniC_lexer_R1
import sys
...
def p_error(p):
    ...
    if VERBOSE:
        if p is not None:
            print
            "Syntax error at line " + str(p.lexer.lineno) + " Unexpected token  " + str(p.value)
        else:
            print
            "Syntax error at line: " + str(cminus_lexer.lexer.lineno)
    else:
